...
# Définition du chemin vers le modèle MLflow
model_uri = "runs:/62c9722eb896400dabe73d9302cddea7/model"

# Initialisation de l'app FastAPI
app = FastAPI(title="Sentiment Analysis API")

# Sélection du modèle en fonction de l'environnement
if os.getenv("TEST_ENV") != "1":
    # Chargement du vrai modèle MLflow
    model = mlflow.pyfunc.load_model(model_uri)
else:
    # Modèle factice pour les tests
    class DummyModel:
        def predict(self, data):
            return ["dummy_prediction"] * len(data)
    model = DummyModel()

# Schéma des données reçues par l'API
...
def predict(request: PredictionRequest):
    # Ici, on suppose que ton modèle prend une liste de textes
